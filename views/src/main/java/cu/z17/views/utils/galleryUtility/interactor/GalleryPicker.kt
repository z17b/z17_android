package cu.z17.views.utils.galleryUtility.interactor
...
import cu.z17.views.utils.galleryUtility.GalleryImage
import cu.z17.views.utils.galleryUtility.GalleryVideo
import java.io.File

class GalleryPicker(var ctx: Context) {

    suspend fun getImages(limit: Int, offset: Int): ArrayList<GalleryImage> {
        ...
        try {
            if (isReadWritePermitted()) {
                ...
                val imagescursor = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    val bundle = bundleOf(
    ...
    suspend fun getVideos(limit: Int, offset: Int): ArrayList<GalleryVideo> {
        ...
        try {
            if (isReadWritePermitted()) {
                ...
                val videocursor = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    val bundle = bundleOf(
                        ContentResolver.QUERY_ARG_OFFSET to offset,
                        ContentResolver.QUERY_ARG_LIMIT to limit,
    ...
    private fun isReadWritePermitted2(): Boolean =
