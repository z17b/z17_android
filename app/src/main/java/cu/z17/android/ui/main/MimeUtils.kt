...
object MimeUtils {
    ...
    fun guessMimeTypeFromExtension(extension: String?): String? {
        return if (extension == null || extension.isEmpty()) {
            null
        } else extensionToMimeTypeMap[extension.lowercase(Locale.getDefault())]
    }
    ...
    fun guessExtensionFromMimeType(mimeType: String?): String? {
        return if (mimeType == null || mimeType.isEmpty()) {
            null
        } else mimeTypeToExtensionMap[mimeType.split(";".toRegex()).dropLastWhile { it.isEmpty() }
            .toTypedArray()[0]]
    }

    fun guessMimeTypeFromUri(context: Context, uri: Uri): String {
        ...
        // try the extension
        if (mimeType == null && uri.path != null) {
            val path = uri.path
    ...
    fun extractRelevantExtension(url: URL): String? {
        val path = url.path
        return extractRelevantExtension(path, true)
    }

    @JvmOverloads
    fun extractRelevantExtension(path: String?, ignoreCryptoExtension: Boolean = false): String? {
        ...
        return if (dotPosition != -1) {
            ...
            } else {*/filename.substring(dotPosition + 1)
            //}
        } else null
    }
}
